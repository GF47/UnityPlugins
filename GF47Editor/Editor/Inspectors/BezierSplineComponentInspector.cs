...
namespace GF47Editor.Editor.Inspectors
{
    ...
    public class BezierSplineComponentInspector : UnityEditor.Editor
    {
        ...
        private BezierSplineComponent _target;
        private Vector3[] _points = new Vector3[DRAW_POINTS_COUNT + 1];
        ...
        void OnEnable()
        {
            _target = (BezierSplineComponent)target;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            tmpSmooth = GUILayout.Toggle(tmpSmooth, tmpSmooth ? "Smooth" : "PolyLine", EditorStyles.toolbarButton);
            ...
            for (int i = 0; i < _target.spline.Count; i++)
            {
                GUI.backgroundColor = i == _selectedIndex ? Color.yellow : Color.cyan;
                EditorGUILayout.BeginVertical(EditorStyles.textField);
                BezierPoint point = _target.spline[i];
                if (point == null)
                {
                    //Debug.Log("")
                    _target.spline[i] = new BezierPoint();
                    point = _target.spline[i];
                }
            ...
            if (insertId > -1)
            {
                float percent =(insertId - 0.5f) / (_target.spline.Count - 1);
                BezierResult result = _target.spline.GetResult(percent);
                _target.spline.Insert(insertId, new BezierPoint(result.position, 1f, BezierPoint.PointType.BezierCorner));
#if LAST_ACTIVE_SCENE_VIEW
                SceneView.lastActiveSceneView.Repaint();
#else
                SceneView.currentDrawingSceneView.Repaint();
            ...
            if (removeId > -1)
            {
                _target.spline.RemoveAt(removeId);
#if LAST_ACTIVE_SCENE_VIEW
                SceneView.lastActiveSceneView.Repaint();
#else
                SceneView.currentDrawingSceneView.Repaint();
            ...
            {
                _target.spline.Add(new BezierPoint());
#if LAST_ACTIVE_SCENE_VIEW
                SceneView.lastActiveSceneView.Repaint();
#else
                SceneView.currentDrawingSceneView.Repaint();
        ...
        void OnSceneGUI()
        {
            ...
            for (int i = 0; i < _target.spline.Count - 1; i++)
            {
                ShowPoint(i);
                if (_smooth)
                {
//#if !DRAW_BY_DEFAULT_BEZIER_LINE
                    Handles.DrawBezier(_target.spline[i].Point, _target.spline[i + 1].Point, _target.spline[i].HandleR, _target.spline[i + 1].HandleL, Color.green, null, 2f);
//#endif
                }
            }
            ShowPoint(_target.spline.Count - 1);
            if (!_smooth)
            {
//#if DRAW_BY_DEFAULT_BEZIER_LINE
                ...
                {
                    BezierResult result = _target.spline.GetResult(total);
...
//#endif
        ...
        {
            if (_target.spline.points == null || _target.spline.Count < 1)
            {
                return;
            }
            BezierPoint point = _target.spline[index];
